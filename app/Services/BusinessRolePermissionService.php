...
class BusinessRolePermissionService
{
     ...
     * 4. .own permission (if checking base permission like clients.view, also check clients.view.own)
     *
     * Владелец проверяется по правам роли так же, как остальные (права задаются в роли «Владелец» в настройках).
     */
    public function hasPermission(int $roleId, string $permission): bool
    {
        ...
        // Does role have permission (direct or via wildcard)?
        $permissions = $this->getPermissionsForRole($roleId);
        if ($this->checkPermission($permissions, $permission)) {
            return true;
        }

        // If checking base permission (like clients.view), also check .own variant
