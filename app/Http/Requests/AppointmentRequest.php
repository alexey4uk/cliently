...
class AppointmentRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        ...
        $clientIdRule = ['required', 'exists:clients,id'];
        $serviceIdRule = ['required', 'exists:services,id'];
        $masterIdRule = ['nullable', 'exists:masters,id'];
        $locationIdRule = ['nullable', 'exists:locations,id'];

        if ($business) {
            $clientIdRule = ['required', Rule::exists('clients', 'id')->where('business_id', $business->id)];
            $serviceIdRule = ['required', Rule::exists('services', 'id')->where('business_id', $business->id)];
            $masterIdRule = ['nullable', Rule::exists('masters', 'id')->where('business_id', $business->id)];
            $locationIdRule = ['nullable', Rule::exists('locations', 'id')->where('business_id', $business->id)];
        }

        return [
            'client_id' => $clientIdRule,
            'service_id' => $serviceIdRule,
            'master_id' => $masterIdRule,
            'location_id' => $locationIdRule,
