...
class TicketController extends Controller
{
    ...
    public function show(Ticket $ticket)
    {
        ...
        if (! $ticket->business) {
            return redirect()->route('panel.tickets')
                ->with('error', 'Бизнес этого тикета удалён. Тикет недоступен.');
        }
    ...
    public function edit(Ticket $ticket)
    {
        ...
        if (! $ticket->business) {
            return redirect()->route('panel.tickets')
                ->with('error', 'Бизнес этого тикета удалён. Редактирование недоступно.');
        }
    ...
    public function addComment(TicketCommentRequest $request, Ticket $ticket)
    {
        if (! $ticket->business) {
            return redirect()->route('panel.tickets')
                ->with('error', 'Бизнес этого тикета удалён. Добавление комментариев недоступно.');
        }
