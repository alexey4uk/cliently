...
class AnalyticsController extends Controller
{
    ...
    public function financial(Request $request)
    {
        ...
        return view(
            "panel.analytics.financial",
            compact("data", "filters", "plans"),
        );
    }
    ...
    private function getOverviewData(): array
    {
        ...
        return [
            ...
            // Рост бизнесов
            $newBusinessesLastMonth = Business::whereBetween("created_at", [
                $twoMonthsAgo,
                $monthAgo,
            ])->count();
            $newBusinessesThisMonth = Business::where(
                "created_at",
                ">=",
                $monthAgo,
            )->count();
    ...
    private function getGeneralData(array $filters): array
    {
            ...
            $cancellationRate =
                $total > 0 ? round(($cancelled / $total) * 100, 1) : 0;
            ...
            // Динамика записей
            $appointmentsByPeriod = $this->getAppointmentsByPeriod(
                $startDate,
                $endDate,
                $filters,
            );
            ...
            $statsByBusiness = DB::table("businesses")
                ->leftJoin("appointments", function ($join) use (
                    $startDate,
                    $endDate,
                ) {
                    $join
                        ->on("businesses.id", "=", "appointments.business_id")
            ...
            $statsByService = $serviceStats->map(function ($item) use (
                $services,
            ) {
                return [
                    "service_id" => $item->service_id,
                    "service_name" =>
                        $services[$item->service_id]->name ??
                        "Неизвестная услуга",
                    "count" => $item->count,
                ];
            });
    ...
    private function getSubscriptionsData(array $filters): array
    {
        ...
        $endDate = Carbon::parse($filters["date_to"])->endOfDay();
            ...
            $query = Subscription::whereBetween("created_at", [
                $startDate,
                $endDate,
            ]);
            ...
            // Отмененные подписки
            $cancelledSubscriptions = Subscription::whereNotNull(
                "cancelled_at",
            )->count();

            // Распределение по тарифам
            $distributionByPlan = Subscription::select(
                "plan_id",
                DB::raw("COUNT(*) as count"),
            )
                ->groupBy("plan_id")
            ...
            $trialToPaid = Subscription::where("status", "active")
                ...
                ->count();
            $totalTrials =
                Subscription::where("status", "trial")->count() + $trialToPaid;
            $conversionRate =
                $totalTrials > 0
            ...
            // Динамика подписок
            $subscriptionsByPeriod = $this->getSubscriptionsByPeriod(
                $startDate,
                $endDate,
                $filters,
            );
            ...
            $newSubscriptions = Subscription::whereBetween("created_at", [
                $startDate,
                $endDate,
            ])
                ->with(["user", "plan"])
            ...
            // Отмененные подписки
            $cancelledSubscriptionsList = Subscription::whereNotNull(
                "cancelled_at",
            )
    ...
    private function getChartData(int $days = 30): array
    {
        ...
        $businessesByDate = Business::whereBetween("created_at", [
            $startDate,
            $endDate,
        ])
        ...
        $appointmentsByDate = Appointment::whereBetween("created_at", [
            $startDate,
            $endDate,
        ])
            ->selectRaw("DATE(created_at) as date, COUNT(*) as count")
        ...
        $clientsByDate = Client::whereBetween("created_at", [
            $startDate,
            $endDate,
        ])
            ->selectRaw("DATE(created_at) as date, COUNT(*) as count")
     ...
     */
    private function getRevenueByPeriod(
        Carbon $startDate,
        Carbon $endDate,
        array $filters,
    ): array {
        $query = Invoice::where("status", "paid")->whereBetween("paid_at", [
            $startDate,
            $endDate,
        ]);
     ...
     * Optimized: loads all data with single query instead of N queries per day.
     */
    private function getAppointmentsByPeriod(
        Carbon $startDate,
        Carbon $endDate,
        array $filters,
    ): array {
     ...
     * Optimized: loads all data with single query instead of N queries per day.
     */
    private function getSubscriptionsByPeriod(
        Carbon $startDate,
        Carbon $endDate,
        array $filters,
    ): array {
