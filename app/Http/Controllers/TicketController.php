...
class TicketController extends Controller
{
    ...
    public function create()
    {
        ...
        if (! $business) {
            $settings = config('tickets');
            if (! $settings['enabled']) {
                return redirect()->route('tickets.index')->with('info', 'Система тикетов отключена.');
            }
    ...
    private function findTicketForCurrentUser(int $id): Ticket|RedirectResponse|null
    {
        ...
        if (! $business) {
            return redirect()->route('tickets.index')->with('error', 'Сначала создайте бизнес или примите приглашение.');
        }
