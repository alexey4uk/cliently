    ...
    function show(items) {
        ...
        items.slice(0, LIMIT).forEach((feature) => {
            ...
            btn.addEventListener('click', () => {
                ...
                if (addressInput) addressInput.value = formatFullAddress(feature);
...
function initPerFieldAutocomplete(form) {
    const cityInput = form.querySelector('input[name="city"][data-address-field="city"]');
    const streetInput = form.querySelector('input[name="street"][data-address-field="street"]');
    const houseInput = form.querySelector('input[name="house"][data-address-field="house"]');
    ...
    function attachDropdown(input, getQuery, formatItem, onSelect) {
        ...
        if (!dropdown || dropdown.getAttribute('data-address-suggestions') === null) {
        ...
        input.addEventListener('input', () => {
            ...
            debounceTimer = setTimeout(() => {
                fetchAddressSuggestions(q)
                    .then((features) => {
                        if (input.getAttribute('data-address-field') === 'city') {
                            const seen = new Set();
                            const unique = features.filter((f) => {
                                const key = (f.properties?.city || f.properties?.name || '').trim().toLowerCase();
    ...
    if (cityInput) {
        attachDropdown(
            ...
            (f) => (f.properties?.city || f.properties?.name || '').trim() || 'Город',
            (f) => {
    ...
    if (houseInput) {
        attachDropdown(
            ...
            (f) => {
                ...
                const buildingInput = form.querySelector('input[name="building"]');
                if (buildingInput && p.housenumber) buildingInput.value = buildingInput.value || '';
            }
        );
    }
}
...
function initAddressAutocomplete() {
    document.querySelectorAll('[data-address-autocomplete]').forEach(initSingleSearch);
