...
<div class="max-w-4xl mx-auto px-3 sm:px-6 pb-4 sm:pb-6">
@php
    $hasSlug = $business ? !empty($business->slug) : false;
    ...
    $isEnabled = $business ? ($business->online_booking_enabled ?? false) : false;
    $canEnable = $can_enable_online_booking ?? false;
    $checks = $checks ?? [];
@endphp

<div x-data="{
    isEnabled: {{ $isEnabled ? 'true' : 'false' }},
    canEnable: {{ $canEnable ? 'true' : 'false' }},
    isLoading: false,
    toggleError: '',
    copiedWeb: false,
    ...
    <!-- Чек-лист для включения онлайн-записи -->
    <div class="bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-slate-200 dark:border-slate-800 p-4 sm:p-6 mb-4 sm:mb-6">
        <h2 class="text-base font-semibold text-slate-900 dark:text-white mb-3">Условия для включения онлайн-записи</h2>
        <ul class="space-y-3 text-sm">
            <li class="flex items-start sm:items-center gap-3">
                @if($checks['has_business'] ?? false)
                    <i class="fa-solid fa-circle-check text-emerald-500 shrink-0 mt-0.5"></i>
                    <span class="text-slate-700 dark:text-slate-300 break-words">Бизнес создан</span>
                @else
                    <i class="fa-solid fa-circle-xmark text-slate-400 shrink-0 mt-0.5"></i>
                    <a href="{{ route('settings.business.create') }}" class="text-indigo-600 dark:text-indigo-400 hover:underline break-words">Создайте бизнес</a>
                @endif
            </li>
            <li class="flex items-start sm:items-center gap-3">
                @if($checks['has_slug'] ?? false)
                    <i class="fa-solid fa-circle-check text-emerald-500 shrink-0 mt-0.5"></i>
                    <span class="text-slate-700 dark:text-slate-300 break-words">У бизнеса заполнен идентификатор (slug)</span>
                @else
                    <i class="fa-solid fa-circle-xmark text-slate-400 shrink-0 mt-0.5"></i>
                    @if($business)
                        <a href="{{ route('settings.business.edit') }}" class="text-indigo-600 dark:text-indigo-400 hover:underline break-words">Заполните идентификатор в настройках бизнеса</a>
                    @else
                        <span class="text-slate-600 dark:text-slate-400 break-words">Создайте бизнес и укажите идентификатор</span>
                    @endif
                @endif
            </li>
            <li class="flex items-start sm:items-center gap-3">
                @if($checks['has_locations'] ?? false)
                    <i class="fa-solid fa-circle-check text-emerald-500 shrink-0 mt-0.5"></i>
                    <span class="text-slate-700 dark:text-slate-300 break-words">Добавлена минимум одна локация</span>
                @else
                    <i class="fa-solid fa-circle-xmark text-slate-400 shrink-0 mt-0.5"></i>
                    <a href="{{ route('settings.locations.create') }}" class="text-indigo-600 dark:text-indigo-400 hover:underline break-words">Добавьте локацию</a>
                @endif
            </li>
            <li class="flex items-start sm:items-center gap-3">
                @if($checks['has_services'] ?? false)
                    <i class="fa-solid fa-circle-check text-emerald-500 shrink-0 mt-0.5"></i>
                    <span class="text-slate-700 dark:text-slate-300 break-words">Добавлена минимум одна услуга</span>
                @else
                    <i class="fa-solid fa-circle-xmark text-slate-400 shrink-0 mt-0.5"></i>
                    <a href="{{ route('services.create') }}" class="text-indigo-600 dark:text-indigo-400 hover:underline break-words">Добавьте услугу</a>
                @endif
            </li>
            <li class="flex items-start sm:items-center gap-3">
                @if($checks['has_masters'] ?? false)
                    <i class="fa-solid fa-circle-check text-emerald-500 shrink-0 mt-0.5"></i>
                    <span class="text-slate-700 dark:text-slate-300 break-words">Добавлен минимум один мастер</span>
                @else
                    <i class="fa-solid fa-circle-xmark text-slate-400 shrink-0 mt-0.5"></i>
                    <a href="{{ route('settings.masters.create') }}" class="text-indigo-600 dark:text-indigo-400 hover:underline break-words">Добавьте мастера</a>
                @endif
            </li>
            <li class="flex items-start sm:items-center gap-3">
                @if($checks['has_booking_chain'] ?? false)
                    <i class="fa-solid fa-circle-check text-emerald-500 shrink-0 mt-0.5"></i>
                    <span class="text-slate-700 dark:text-slate-300 break-words">Услуга привязана к мастеру, мастер — к локации</span>
                @else
                    <i class="fa-solid fa-circle-xmark text-slate-400 shrink-0 mt-0.5"></i>
                    <div class="flex flex-wrap gap-x-3 gap-y-1 min-w-0">
                        <span class="text-slate-600 dark:text-slate-400 basis-full sm:basis-auto">Привяжите услуги к мастерам и мастеров к локациям</span>
                        <a href="{{ route('settings.masters') }}" class="text-indigo-600 dark:text-indigo-400 hover:underline">Мастера</a>
                        <a href="{{ route('services.index') }}" class="text-indigo-600 dark:text-indigo-400 hover:underline">Услуги</a>
                    </div>
                @endif
            </li>
        </ul>
    ...
    <div class="bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-slate-200 dark:border-slate-800 p-4 sm:p-6 mb-4 sm:mb-6">
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div class="min-w-0">
                <h1 class="text-xl sm:text-2xl font-bold text-slate-900 dark:text-white break-words">Онлайн-запись</h1>
            ...
            <div class="flex flex-col gap-2 w-full sm:w-auto sm:items-end">
                <div class="flex items-center justify-between sm:justify-end gap-3">
                    ...
                    <div class="min-h-[44px] min-w-[44px] flex items-center justify-center shrink-0 touch-manipulation">
                        <button type="button"
                                role="switch"
                                :aria-checked="isEnabled"
                                aria-label="Включить или выключить онлайн-запись"
                                @click="canEnable ? toggleBooking() : null"
                                :disabled="isLoading || !canEnable"
                                title="{{ $canEnable ? 'Включить или выключить онлайн-запись' : 'Выполните все пункты выше' }}"
                                :class="isEnabled ? 'bg-indigo-600' : 'bg-slate-300 dark:bg-slate-700'"
                                class="relative inline-flex h-8 w-14 sm:h-7 sm:w-12 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 disabled:opacity-50 disabled:cursor-not-allowed">
                            <span :class="isEnabled ? 'translate-x-7 sm:translate-x-6' : 'translate-x-1'"
                                  class="inline-block h-6 w-6 sm:h-5 sm:w-5 transform rounded-full bg-white transition-transform shadow-sm"></span>
                        </button>
                    </div>
                </div>
                @if(!$canEnable && $business)
                <p class="text-xs text-amber-600 dark:text-amber-400 w-full sm:w-auto order-last">Выполните все пункты выше, чтобы включить онлайн-запись.</p>
                @endif
                <p x-show="toggleError" x-text="toggleError" x-cloak
                   class="text-sm text-rose-600 dark:text-rose-400 break-words"
        ...
        <div class="space-y-6">
            ...
            <div class="bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-slate-200 dark:border-slate-800 p-4 sm:p-6">
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <div class="flex flex-col gap-3 sm:flex-row sm:items-center p-3 sm:p-4 bg-slate-50 dark:bg-slate-800/50 rounded-lg border border-slate-200 dark:border-slate-700">
                            <div class="flex-1 min-w-0 overflow-hidden">
                                <p class="text-xs sm:text-sm text-slate-700 dark:text-slate-300 font-mono break-all">
                                    {{ $displayUrl }}
                                </p>
                            </div>
                            <button type="button"
            ...
            <div class="bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-slate-200 dark:border-slate-800 p-4 sm:p-6">
                <div class="flex items-center gap-3 mb-4">
                    <div class="h-10 w-10 shrink-0 rounded-lg bg-amber-100 dark:bg-amber-500/20 flex items-center justify-center">
                        <i class="fa-solid fa-lightbulb text-amber-600 dark:text-amber-400"></i>
                    </div>
                    <h3 class="text-base sm:text-lg font-semibold text-slate-900 dark:text-white break-words">Идеи для размещения QR-кодов</h3>
                </div>
    ...
    <template x-if="!isEnabled">
        <div class="bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-slate-200 dark:border-slate-800 p-4 sm:p-8 text-center">
            <div class="max-w-md mx-auto">
                <div class="h-16 w-16 sm:h-20 sm:w-20 rounded-xl bg-amber-100 dark:bg-amber-500/20 flex items-center justify-center mx-auto mb-4">
                    <i class="fa-solid fa-toggle-off text-amber-600 dark:text-amber-400 text-xl sm:text-2xl"></i>
                </div>
    ...
    <template x-if="isEnabled && !{{ $hasSlug ? 'true' : 'false' }}">
        <div class="bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-slate-200 dark:border-slate-800 p-4 sm:p-8 text-center">
            <div class="max-w-md mx-auto">
                <div class="h-16 w-16 sm:h-20 sm:w-20 rounded-xl bg-rose-100 dark:bg-rose-500/20 flex items-center justify-center mx-auto mb-4">
                ...
                <a href="{{ route('settings.business.edit') }}"
                   class="inline-flex items-center justify-center gap-2 px-6 py-3 min-h-[48px] text-sm font-medium text-white bg-indigo-600 hover:bg-indigo-700 rounded-lg transition-colors touch-manipulation">
    ...
    <div x-show="showQrModal"
         ...
         class="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-6 bg-black/50 backdrop-blur-sm overflow-y-auto overscroll-contain"
         ...
         style="display: none;">
        <div @click.stop
             class="relative bg-white dark:bg-slate-900 rounded-xl shadow-xl w-full max-w-[min(100vw-1.5rem,28rem)] max-h-[90vh] overflow-y-auto p-4 sm:p-6 my-auto"
             x-transition:enter="transition ease-out duration-200"
            ...
            <div class="flex flex-col items-center gap-4">
                <div class="bg-white p-3 sm:p-4 rounded-xl border-2 border-slate-200 dark:border-slate-700 shadow-lg">
                    <img :src="qrImageUrl" :alt="qrTitle" class="w-56 h-56 sm:w-64 sm:h-64">
                </div>
                ...
                <div class="w-full space-y-3">
                    <div class="bg-slate-50 dark:bg-slate-800/50 rounded-lg p-3">
                        ...
                        <p class="text-xs text-slate-700 dark:text-slate-300 font-mono break-all"
                    ...
                    <div class="flex flex-col-reverse sm:flex-row gap-3">
                        <button type="button"
                                @click="showQrModal = false"
                                class="w-full sm:w-auto min-h-[44px] px-4 py-2.5 text-sm font-medium text-slate-700 dark:text-slate-300 bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700 rounded-lg transition-colors touch-manipulation">
                            Закрыть
                        </button>
                        <button type="button"
                                @click="downloadModalQr()"
                                :disabled="isDownloading"
                                class="w-full sm:flex-1 min-h-[44px] inline-flex items-center justify-center gap-2 px-4 py-2.5 text-sm font-medium text-white bg-indigo-600 hover:bg-indigo-700 rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed touch-manipulation">
                            <i class="fa-solid fa-download text-sm"></i>
                            <span x-text="isDownloading ? 'Скачивается...' : 'Скачать'"></span>
                        </button>
                    </div>
