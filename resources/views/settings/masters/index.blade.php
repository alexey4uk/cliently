        ...
        @if ($masters->count() > 0)
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
                @foreach ($masters as $master)
                    ...
                    <div
                        class="bg-white dark:bg-slate-900 rounded-lg border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden hover:shadow-md transition-all h-full flex flex-col active:scale-[0.99]">
                        ...
                        <div class="px-4 md:px-5 py-4 border-b border-slate-200 dark:border-slate-800 flex-shrink-0">
                            <div class="flex items-center gap-3">
                                <div
                                    ...
                                    <i class="fa-solid fa-user-tie text-indigo-600 dark:text-indigo-400 text-base"></i>
                                </div>
                                <div class="min-w-0 flex-1">
                                    <h3 class="text-base font-semibold text-slate-900 dark:text-white truncate mb-0.5">
                        ...
                        <div class="p-4 md:p-5 space-y-4 flex-1">
                            ...
                            <div class="space-y-2">
                                <div class="flex items-start gap-3">
                                    <i class="fa-solid fa-phone text-indigo-400 text-sm mt-0.5 flex-shrink-0"></i>
                                    <div class="min-w-0 flex-1">
                                ...
                                @if ($master->email)
                                    <div class="flex items-start gap-3">
                                        <i class="fa-solid fa-envelope text-indigo-400 text-sm mt-0.5 flex-shrink-0"></i>
                                        <div class="min-w-0 flex-1">
                            ...
                            @if ($workingHours)
                                <div class="flex items-start gap-3">
                                    <i class="fa-solid fa-clock text-indigo-400 text-sm mt-0.5 flex-shrink-0"></i>
                                    <div class="min-w-0 flex-1">
                            ...
                            <div class="flex flex-wrap gap-2 pt-2 border-t border-slate-200 dark:border-slate-800">
                                @if ($master->locations->count() > 0)
                                    <div
                                        class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-indigo-100 dark:bg-indigo-900/50 text-indigo-700 dark:text-indigo-300 text-xs font-medium">
                                        <i class="fa-solid fa-location-dot text-xs"></i>
                                        <span>{{ $master->locations->count() }}
                                            {{ $master->locations->count() === 1 ? 'локация' : 'локаций' }}</span>
                                    </div>
                                @endif
                                @if ($master->services->count() > 0)
                                    <div
                                        class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-purple-100 dark:bg-purple-900/50 text-purple-700 dark:text-purple-300 text-xs font-medium">
                                        <i class="fa-solid fa-scissors text-xs"></i>
                                        <span>{{ $master->services->count() }}
                                            {{ $master->services->count() === 1 ? 'услуга' : 'услуг' }}</span>
                                    </div>
                                @endif
                                @if ($master->locations->count() === 0 && $master->services->count() === 0)
                                    <span class="text-xs text-slate-400 dark:text-slate-500 italic px-2 py-1">Не
                                        назначены</span>
                                @endif
                            </div>
                        </div>
                        ...
                        <!-- Действия -->
                        <div
                            class="px-4 md:px-5 py-3 border-t border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/30 flex-shrink-0">
                            <div class="flex items-center justify-end gap-2">
                                ...
                                <form method="POST" action="{{ route('settings.masters.destroy', $master) }}"
                                    ...
                                    <!-- Кнопка Удалить -->
                                    <button type="button"
                                        @click="openDeleteModal({{ $master->id }}, '{{ addslashes($master->first_name) }}')"
        ...
        @else
            ...
            <div
                ...
                <div class="max-w-sm mx-auto">
                    {{-- Иконка с градиентом --}}
                    <div
                        class="h-16 w-16 rounded-2xl bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center mx-auto mb-4 shadow-md">
                    ...
                    {{-- Кнопка действия --}}
                    <a href="{{ route('settings.masters.create') }}"
                        class="inline-flex items-center justify-center gap-2 px-6 py-3 text-sm font-medium text-white bg-indigo-600 hover:bg-indigo-700 active:bg-indigo-800 rounded-xl transition-all shadow-md hover:shadow-lg active:scale-95 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2">
                        <i class="fa-solid fa-plus text-xs"></i>
                        <span>Добавить мастера</span>
                    </a>
                </div>
            ...
            <div @click.stop x-transition:enter="transition ease-out duration-300"
                ...
                <div class="px-4 md:px-6 py-4 md:py-5">
                    <p class="text-sm md:text-base text-slate-700 dark:text-slate-300 mb-6">
                        Вы уверены, что хотите удалить мастера <span class="font-semibold" x-text="masterName"></span>? Это
