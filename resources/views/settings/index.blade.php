...
<div class="max-w-6xl mx-auto">
    @if($business && ($canViewBusinesses ?? false))
    <!-- Карточка: Бизнесы -->
    <a href="{{ route('settings.businesses.index') }}"
       class="group flex items-center gap-4 bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-slate-200 dark:border-slate-800 p-6 mb-6 hover:shadow-md hover:border-indigo-400 dark:hover:border-indigo-600 transition-all">
        <div class="h-12 w-12 rounded-lg bg-indigo-100 dark:bg-indigo-500/20 flex items-center justify-center group-hover:bg-indigo-200 dark:group-hover:bg-indigo-500/30 transition-colors shrink-0">
            <i class="fa-solid fa-briefcase text-indigo-600 dark:text-indigo-400 text-lg"></i>
        </div>
        <div class="min-w-0 flex-1">
            <h2 class="text-lg font-semibold text-slate-900 dark:text-white group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors">Бизнесы</h2>
            <p class="text-sm text-slate-600 dark:text-slate-400 mt-0.5">Переключение между проектами, добавление и редактирование</p>
        </div>
        <i class="fa-solid fa-arrow-right text-slate-400 group-hover:text-indigo-500 transition-colors shrink-0"></i>
    </a>
    @endif
    ...
    <div class="bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-slate-200 dark:border-slate-800 p-6 mb-6">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            ...
            @if($business && ($canUpdateBusiness ?? false))
            <a href="{{ route('settings.business.edit') }}"
               class="inline-flex items-center gap-2 px-4 py-2.5 text-sm font-medium text-slate-700 dark:text-slate-300 bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-700 transition-colors shrink-0">
    ...
    <div class="mb-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            @php
                $isBookingEnabled = $business->online_booking_enabled ?? false;
                $obPivot = \Illuminate\Support\Facades\DB::table('business_user')
                    ->where('business_id', $business->id)
                    ->where('user_id', Auth::id())
                    ->first();
                $hasOnlineBookingAccess = $obPivot && $obPivot->role_id
                    && app(\App\Services\BusinessRolePermissionService::class)->hasPermission($obPivot->role_id, 'client.online_booking.manage');
            @endphp
            @if($hasOnlineBookingAccess)
