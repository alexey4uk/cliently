            ...
            <div class="bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-slate-200 dark:border-slate-800 p-6">
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <div class="flex flex-wrap gap-4">
                            <label class="inline-flex items-center gap-2 cursor-pointer">
                                <input type="radio" name="type" value="organization"
                                    {{ old('type', 'organization') === 'organization' ? 'checked' : '' }}
                        ...
                        <p class="mt-1 text-xs text-slate-500 dark:text-slate-400">Организация — салон или компания; Мастер — вы работаете самостоятельно.</p>
                        @error('type')
                            <p class="mt-1 text-sm text-rose-600 dark:text-rose-400">{{ $message }}</p>
                        @enderror
                    </div>
...
(function() {
    ...
    function sanitize(s) {
        if (!s) return '';
        // Нормализуем дефисоподобные символы (en-dash, em-dash, minus sign и т.д.) в обычный дефис
        s = s.replace(/[\u2010-\u2014\u2212\u00AD]/g, '-');
        // Убираем только ведущий дефис; завершающий оставляем, чтобы при вводе "ip-" дефис не пропадал
