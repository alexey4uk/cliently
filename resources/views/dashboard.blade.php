    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-4">
        ...
        <div class="lg:col-span-2 order-2 lg:order-1">
                    ...
                    <div>
                        ...
                        <div class="flex items-center gap-2 mt-0.5">
                            ...
                            @if($totalToday > 0)
                                ...
                                <span class="text-xs text-slate-500 dark:text-slate-400">{{ $completedCount }}/{{ $totalToday }}</span>
                            @endif
                        </div>
                    </div>
                ...
                <div>
                    @if($upcomingAppointments->isNotEmpty())
                        @foreach($upcomingAppointments as $appointment)
                            <div class="p-3 border-b border-slate-200 dark:border-slate-700 last:border-0 hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                                <div class="flex items-center justify-between gap-3">
                                    <div class="flex-1 min-w-0">
                                        ...
                                        <div class="flex items-center gap-3 text-xs text-slate-600 dark:text-slate-400">
                                            <span class="truncate">{{ $appointment->client->full_name }}</span>
                                ...
                                <div class="flex items-center gap-1.5 flex-shrink-0">
                                    <button @click="openModal('{{ $appointment->client->phone }}', '{{ $appointment->client->phone }}', '{{ $appointment->client->full_name }}')"
                                        class="h-7 w-7 rounded-md bg-indigo-50 dark:bg-indigo-500/20 text-indigo-700 dark:text-indigo-300 hover:bg-indigo-100 dark:hover:bg-indigo-500/30 transition-colors flex items-center justify-center">
                                        <i class="fa-solid fa-phone text-xs"></i>
                                    </button>
                                    <div x-data="{ open: false }" class="relative">
                                        <button @click="open = !open"
                                            class="h-7 w-7 rounded-md bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors flex items-center justify-center">
                                        ...
                                        <div x-show="open" 
                                            ...
                                            style="display: none;">
                                            <a href="{{ route('appointments.show', $appointment->id) }}" class="block px-4 py-2 text-sm text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700">
                    ...
                    @if($completedAppointments->isNotEmpty())
                        <div class="px-4 pt-3 pb-1.5 border-t border-slate-200 dark:border-slate-700">
                            <p class="text-xs font-medium text-slate-500 dark:text-slate-400 uppercase tracking-wide">Выполненные</p>
                        </div>
                        @foreach($completedAppointments as $appointment)
                            <div class="p-3 border-b border-slate-200 dark:border-slate-700 last:border-0 hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors opacity-75">
                                <div class="flex items-center justify-between gap-3">
                                    <div class="flex-1 min-w-0">
                                        <div class="flex items-center gap-2.5 mb-1.5">
                                            <span class="text-sm font-semibold text-slate-500 dark:text-slate-400 line-through">
                                                {{ \Carbon\Carbon::parse($appointment->time)->format('H:i') }}
                                            </span>
                                            <span class="text-sm font-medium text-slate-500 dark:text-slate-400 truncate">{{ $appointment->service->name }}</span>
                                            <span class="inline-flex items-center px-1.5 py-0.5 rounded text-xs font-medium bg-emerald-100 dark:bg-emerald-500/20 text-emerald-700 dark:text-emerald-300">
                                                Выполнено
                                            </span>
                                        </div>
                                    ...
                                    <div class="flex items-center gap-1.5 flex-shrink-0">
                                        <a href="{{ route('appointments.show', $appointment->id) }}" class="h-7 w-7 rounded-md bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors flex items-center justify-center">
                    ...
                    @if($todayAppointments->isEmpty())
                        <div class="p-8 text-center">
                            ...
                            <p class="text-sm text-slate-500 dark:text-slate-400 mb-3">Нет записей на сегодня</p>
            ...
            @if($pendingAppointments->isNotEmpty())
                <div class="mt-4 bg-white dark:bg-slate-900 rounded-lg border border-slate-200 dark:border-slate-800 shadow-sm">
                    <div class="flex items-center justify-between px-4 py-3 border-b border-slate-200 dark:border-slate-700">
                        <h3 class="text-sm font-semibold text-slate-900 dark:text-white flex items-center gap-2">
                            ...
                            @if($pendingAppointments->count() > 0)
                                <span class="inline-flex items-center justify-center px-1.5 py-0.5 rounded-full text-xs font-medium bg-amber-100 dark:bg-amber-500/20 text-amber-700 dark:text-amber-300">
                    ...
                    <div>
                        @foreach($pendingAppointments as $appointment)
                            <a href="{{ route('appointments.show', $appointment->id) }}" class="block p-3 border-b border-slate-200 dark:border-slate-700 last:border-0 hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                                <div class="flex items-center justify-between gap-3">
                                    <div class="flex-1 min-w-0">
                                        ...
                                        <p class="text-sm font-medium text-slate-700 dark:text-slate-300 truncate mb-0.5">{{ $appointment->service->name }}</p>
        ...
        <div class="space-y-4 order-1 lg:order-2">
            <!-- Следующая запись -->
            @if($nextAppointment)
                <a href="{{ route('appointments.show', $nextAppointment->id) }}" class="block bg-gradient-to-br from-indigo-50 to-indigo-100 dark:from-indigo-500/10 dark:to-indigo-600/10 rounded-lg border border-indigo-200 dark:border-indigo-500/20 shadow-sm p-3 hover:shadow-md transition-all">
                    <h3 class="text-xs font-semibold text-indigo-700 dark:text-indigo-300 mb-2 uppercase tracking-wide">Следующая запись</h3>
            ...
            @elseif($upcomingAppointments->isEmpty() && $completedAppointments->isNotEmpty())
                <div class="bg-white dark:bg-slate-900 rounded-lg border border-slate-200 dark:border-slate-800 shadow-sm p-3">
                    <h3 class="text-xs font-semibold text-slate-500 dark:text-slate-400 mb-2 uppercase tracking-wide">Сегодня</h3>
                    <div class="space-y-1">
            ...
            <!-- Быстрые действия -->
            <div class="bg-white dark:bg-slate-900 rounded-lg border border-slate-200 dark:border-slate-800 shadow-sm p-3">
                <h3 class="text-xs font-semibold text-slate-500 dark:text-slate-400 mb-2 uppercase tracking-wide">Быстрые действия</h3>
                <div class="space-y-1.5">
                    <a href="{{ route('appointments.create') }}" class="w-full inline-flex items-center justify-center gap-1.5 px-3 py-2 bg-indigo-600 text-white rounded-lg text-xs font-medium hover:bg-indigo-700 transition-colors">
                        <i class="fa-solid fa-plus text-xs"></i>
                        <span>Новая запись</span>
                    </a>
                    <a href="{{ route('clients.create') }}" class="w-full inline-flex items-center justify-center gap-1.5 px-3 py-2 bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 rounded-lg text-xs font-medium hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors">
