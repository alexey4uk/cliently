...
@section('content')
    ...
    <div class="max-w-4xl mx-auto">
            ...
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div>
                    <div class="flex items-center gap-3 flex-wrap">
                        <h1 class="text-2xl font-bold text-slate-900 dark:text-white">Запись</h1>
                        <span
                    ...
                    <p class="text-slate-600 dark:text-slate-400 mt-1">
                        Создана {{ $appointment->created_at->format('d.m.Y') }} в
            ...
            <div class="bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-slate-200 dark:border-slate-800 p-6">
                ...
                <div class="space-y-4">
                    ...
                    <div class="flex items-start">
                        <div
                            class="shrink-0 w-10 h-10 bg-indigo-100 dark:bg-indigo-500/20 rounded-lg flex items-center justify-center">
                    ...
                    <div class="flex items-start">
                        <div
                            class="shrink-0 w-10 h-10 bg-purple-100 dark:bg-purple-500/20 rounded-lg flex items-center justify-center">
                            <i class="fa-solid fa-scissors text-purple-600 dark:text-purple-400"></i>
                        </div>
                        <div class="ml-4">
                            <p class="text-sm font-medium text-slate-900 dark:text-white">
                                {{ $appointment->service && !$appointment->service->trashed() ? $appointment->service->name : 'Услуга удалена' }}
                            </p>
                            <p class="text-xs text-slate-500 dark:text-slate-400">Длительность:
                                {{ $appointment->final_duration }} мин</p>
                        </div>
                    </div>
                    ...
                    <div class="flex items-start">
                        <div
                            ...
                            <i
                                class="fa-solid fa-user-tie {{ $appointment->master ? 'text-emerald-600 dark:text-emerald-400' : 'text-amber-600 dark:text-amber-400' }}"></i>
                        </div>
                        <div class="ml-4 flex-1 min-w-0">
                            <p class="text-sm text-slate-500 dark:text-slate-400">Мастер</p>
                            @if ($appointment->master && !$appointment->master->trashed())
                                <p class="text-sm font-medium text-slate-900 dark:text-white">
                                    {{ $appointment->master->name }}</p>
                            @else
                                <p class="text-sm font-medium text-amber-700 dark:text-amber-300">Не назначен</p>
                                @if (isset($canUpdateAppointments) &&
                                        $canUpdateAppointments &&
                                        $appointment->status !== 'cancelled' &&
                                        $appointment->status !== 'completed')
                                    @if (isset($mastersForAssign) && $mastersForAssign->isNotEmpty())
                                        <form id="assign-master" method="POST"
                                            action="{{ route('appointments.assign-master', ['appointment' => $appointment, 'from' => 'show']) }}"
                                            class="mt-2 flex flex-wrap items-center gap-2">
                                            @csrf
                                            @method('PATCH')
                                            <select name="master_id" required
                                                class="rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 text-slate-900 dark:text-white text-sm py-1.5 px-2 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500">
                                                <option value="">Выбрать</option>
                                                @foreach ($mastersForAssign as $m)
                                                    <option value="{{ $m->id }}"
                                                        {{ old('master_id') == $m->id ? 'selected' : '' }}>
                                                        {{ $m->name }}</option>
                                                @endforeach
                                            </select>
                                            <button type="submit"
                                                class="inline-flex items-center gap-1.5 px-2.5 py-1.5 text-xs font-medium text-white bg-indigo-600 hover:bg-indigo-700 rounded-lg transition-colors">
                                        ...
                                        @enderror
                                    @else
                                        <p class="text-xs text-slate-500 dark:text-slate-400 mt-1">Нет мастеров со свободным
                                            слотом на это время.</p>
                                        <a href="{{ route('appointments.edit', $appointment) }}?assign=1"
                                            class="inline-flex items-center gap-1.5 mt-1.5 text-xs font-medium text-indigo-600 dark:text-indigo-400 hover:text-indigo-700 dark:hover:text-indigo-300">
                    ...
                    @if ($appointment->location && !$appointment->location->trashed())
                        <div class="flex items-start">
                            <div
                                class="shrink-0 w-10 h-10 bg-amber-100 dark:bg-amber-500/20 rounded-lg flex items-center justify-center">
                                <i class="fa-solid fa-location-dot text-amber-600 dark:text-amber-400"></i>
                            </div>
                            <div class="ml-4">
                                <p class="text-sm font-medium text-slate-900 dark:text-white">
                                    {{ $appointment->location->name }}</p>
                ...
                @if ($appointment->notes)
                    <div class="mt-6 pt-6 border-t border-slate-200 dark:border-slate-800">
                        ...
                        <p class="text-sm text-slate-600 dark:text-slate-400 whitespace-pre-wrap">{{ $appointment->notes }}
                        </p>
                    </div>
                @endif
            </div>
            ...
            <div class="bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-slate-200 dark:border-slate-800 p-6">
                ...
                @if ($appointment->client && !$appointment->client->trashed())
                    <div class="flex items-center mb-4">
                        <div class="w-12 h-12 rounded-full bg-slate-200 dark:bg-slate-700 flex items-center justify-center text-2xl font-semibold text-slate-600 dark:text-slate-300">
                ...
                @elseif($appointment->client && $appointment->client->trashed())
                    <div
                        class="flex items-center mb-4 p-4 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-lg">
                        <div
                            class="w-16 h-16 rounded-full bg-slate-300 dark:bg-slate-700 flex items-center justify-center shrink-0">
                    ...
                    @if ($canUpdateAppointments && ($appointment->status !== 'cancelled' && $appointment->status !== 'completed'))
                        ...
                        @if ($appointment->status === 'pending')
                            ...
                            @if ($appointment->client?->telegram_user_id)
                                <form method="POST"
                                    action="{{ route('appointments.send-telegram-confirmation', $appointment) }}"
                                    class="inline">
                                    @csrf
                                    <button type="submit"
                        ...
                        @if ($appointment->status !== 'completed')
                            <form method="POST" action="{{ route('appointments.complete', $appointment) }}"
                                ...
                                @method('PATCH')
                                <button type="submit"
                                    class="inline-flex items-center gap-2 px-4 py-2 text-sm font-medium text-blue-600 dark:text-white bg-green-200 dark:bg-green-500/80 border border-blue-200 dark:border-blue-600 rounded-lg hover:bg-blue-100 dark:hover:bg-blue-500/30 transition-colors">
                                    <i class="fa-solid fa-check-circle text-sm"></i>
                                    <span>Завершить</span>
                                </button>
                            </form>
                        ...
                        @if ($appointment->status !== 'cancelled')
                            <form method="POST" action="{{ route('appointments.cancel', $appointment) }}"
                                class="inline" onsubmit="return confirm('Вы уверены, что хотите отменить эту запись?');">
                                @csrf
                                @method('PATCH')
                                <button type="submit"
        ...
        <!-- Activity Log -->
        {{-- <div
            class="mt-6 bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-slate-200 dark:border-slate-800 p-6">
                ...
                @foreach ($statusHistory as $entry)
                    <div class="flex items-start">
                        ...
                        <div class="ml-3">
                            ...
                            <p class="text-xs text-slate-500 dark:text-slate-400">
                                {{ $entry['date']->format('d.m.Y') }} в {{ $entry['date']->format('H:i') }} ·
                                {{ $entry['source'] }}
                            </p>
                        </div>
                    </div>
