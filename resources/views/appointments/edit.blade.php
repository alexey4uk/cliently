...
@section('content')
    ...
    <div class="max-w-3xl mx-auto">
            ...
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div>
                    ...
                    <p class="text-slate-600 dark:text-slate-400 mt-1">
                        {{ $appointment->date->format('d.m.Y') }} в
        ...
        <form id="appointment-edit-form" method="POST" action="{{ route('appointments.update', $appointment) }}"
            class="space-y-6">
            @csrf
            @method('PATCH')
            ...
            <div class="bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-slate-200 dark:border-slate-800 p-6">
                ...
                @if ($appointment->client && !$appointment->client->trashed())
                    <div class="flex items-center p-4 bg-slate-50 dark:bg-slate-800 rounded-lg">
                        <div class="w-12 h-12 rounded-full bg-slate-200 dark:bg-slate-700 flex items-center justify-center mx-auto text-2xl font-semibold text-slate-600 dark:text-slate-300">
                ...
                @elseif($appointment->client && $appointment->client->trashed())
                    <div
                        class="flex items-center p-4 bg-slate-50 dark:bg-slate-800 rounded-lg border border-amber-200 dark:border-amber-800">
                        <div
                            class="w-12 h-12 rounded-full bg-slate-300 dark:bg-slate-700 flex items-center justify-center flex-shrink-0">
        ...
        return [
            ...
            'duration' => $service->duration,
                        ...
                        <div class="relative">
                            ...
                            <button type="button" @click="toggleOpen()"
                                class="w-full pl-10 pr-10 py-2.5 text-sm rounded-lg border {{ $errors->has('service_id') ? 'border-rose-500' : 'border-slate-300 dark:border-slate-700' }} bg-white dark:bg-slate-900 text-left focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-colors cursor-pointer">
                                <span x-show="selectedService" x-cloak class="text-slate-900 dark:text-white">
                                    <span x-text="selectedService ? selectedService.name : ''"></span> - <span
                                        x-text="selectedService ? selectedService.price : ''"></span> BYN (<span
                                    ...
                                    <button type="button" @click="selectService(service)"
                                        ...
                                        <div class="flex-1 min-w-0">
                                            ...
                                            <div class="text-xs text-slate-500 dark:text-slate-400 truncate"
                                                x-text="`${service.price} BYN • ${service.duration} мин`"></div>
                                        </div>
                                        <i x-show="selectedService && selectedService.id === service.id"
                                            class="fa-solid fa-check text-indigo-600 dark:text-indigo-400 text-sm shrink-0"></i>
                                    </button>
                                </template>
                            </div>
                        </div>
            ...
            <div class="bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-slate-200 dark:border-slate-800 p-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        <label for="master_id"
                            class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5">Мастер</label>
                        <div x-data="{
                            open: false,
                            search: '',
                            selectedMaster: null,
        ...
        return [
            ...
            'full_name' => trim($master->first_name . ' ' . ($master->last_name ?? '')),
                            ...
                            <div class="relative">
                                ...
                                <button type="button" @click="toggleOpen()"
                                    class="w-full pl-10 pr-10 py-2.5 text-sm rounded-lg border {{ $errors->has('master_id') ? 'border-rose-500' : 'border-slate-300 dark:border-slate-700' }} bg-white dark:bg-slate-900 text-left focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-colors cursor-pointer">
                    ...
                    <div>
                        <label for="location_id"
                            class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5">Локация</label>
                        <div x-data="{
                            open: false,
                            search: '',
        ...
        return [
            ...
            'phone' => $location->phone,
                            ...
                            <div class="relative ">
                                ...
                                <button type="button" @click="toggleOpen()"
                                    class="w-full pl-10 pr-10 py-2.5 text-sm rounded-lg border {{ $errors->has('location_id') ? 'border-rose-500' : 'border-slate-300 dark:border-slate-700' }} bg-white dark:bg-slate-900 text-left focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-colors cursor-pointer">
            ...
            <div class="bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-slate-200 dark:border-slate-800 p-6">
                ...
                <div class="space-y-4">
                    <div>
                        <label for="status"
                            class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5">Статус</label>
                        <div class="relative">
                            @if ($appointment->date->setTimeFromTimeString($appointment->time)->isPast() || $appointment->status === 'pending' || $appointment->status === 'confirmed' || $appointment->status === 'confirmed')
                                <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                                    <i class="fa-solid fa-info-circle text-slate-400"></i>
                                </div>
                                <select id="status" name="status"class="w-full pl-10 pr-10 py-2.5 text-sm rounded-lg border {{ $errors->has('status') ? 'border-rose-500' : 'border-slate-300 dark:border-slate-700' }} bg-white dark:bg-slate-900 text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-colors appearance-none cursor-pointer">
                                    @if ($appointment->status === 'pending')
                                        <option value="pending"{{ old('status', $appointment->status) === 'pending' ? 'selected' : '' }}>Ожидает</option>
                                    @endif

                                    @if ($appointment->status === 'pending')
                                        <option value="confirmed"{{ old('status', $appointment->status) === 'confirmed' ? 'selected' : '' }}>Подтверждён</option>
                                    @endif
                                    

                                    @if ($appointment->status === 'confirmed')
                                        <option value="completed"{{ old('status', $appointment->status) === 'completed' ? 'selected' : '' }}>Завершён</option>
                                    @endif

                                    <option value="cancelled"{{ old('status', $appointment->status) === 'cancelled' ? 'selected' : '' }}>Отменён</option>
                                </select>
                            @else
                    ...
                    <div>
                        <label for="notes"
                            class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1.5">Заметки</label>
                            ...
                            if (data.success && data.slots && data.slots.length > 0) {
                                ...
                                if (currentOldTime) {
                                    timeSelect.innerHTML =
                                        `<option value="${currentOldTime}" selected>${currentOldTime} (текущее)</option>`;
                                } else {
