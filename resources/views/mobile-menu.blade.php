<nav
    ...
    <div class="mx-auto max-w-6xl flex justify-around py-2.5 text-xs">
        <a href="{{ route('dashboard') }}" class="flex flex-col items-center gap-1 {{ Request::routeIs('dashboard') ? 'text-[#6366F1]' : 'text-slate-500 dark:text-slate-400' }} active:opacity-70 transition-opacity">
            <span
                class="h-7 w-7 rounded-lg {{ Request::routeIs('dashboard') ? 'bg-[#6366F1]/15' : 'bg-slate-100 dark:bg-slate-800' }} flex items-center justify-center {{ Request::routeIs('dashboard') ? 'text-[#6366F1]' : 'text-slate-600 dark:text-slate-400' }}">
                <i class="fa-solid fa-chart-line text-sm"></i>
            </span>
            <span class="font-medium">Дашборд</span>
        </a>
        <a href="#" class="flex flex-col items-center gap-1 text-slate-500 dark:text-slate-400 active:opacity-70 transition-opacity hover:text-slate-700 dark:hover:text-slate-300">
            <span
                class="h-7 w-7 rounded-lg bg-slate-100 dark:bg-slate-800 flex items-center justify-center text-slate-600 dark:text-slate-400">
                <i class="fa-solid fa-calendar-check text-sm"></i>
            </span>
            <span>Записи</span>
        </a>
        <a href="{{ route('clients.index') }}" class="flex flex-col items-center gap-1 {{ Request::routeIs('clients.*') ? 'text-[#6366F1]' : 'text-slate-500 dark:text-slate-400' }} active:opacity-70 transition-opacity">
            <span
                class="h-7 w-7 rounded-lg {{ Request::routeIs('clients.*') ? 'bg-[#6366F1]/15' : 'bg-slate-100 dark:bg-slate-800' }} flex items-center justify-center {{ Request::routeIs('clients.*') ? 'text-[#6366F1]' : 'text-slate-600 dark:text-slate-400' }}">
