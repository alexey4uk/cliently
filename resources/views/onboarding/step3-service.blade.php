        ...
        <div class="space-y-6">
            ...
            <div class="space-y-5">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <div>
                        <label for="duration" class="flex items-center gap-1.5 md:gap-2 text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">
                            <span>Длительность (минуты)*</span>
                        </label>
                        <div class="relative">
                            <input type="number" id="duration" name="duration" required min="15" step="15" value="{{ old('duration', 60) }}"
                                   class="w-full px-2.5 md:px-3 py-2 md:py-2.5 text-sm rounded-md border {{ $errors->has('duration') ? 'border-rose-500 focus:ring-rose-500' : 'border-slate-300 dark:border-slate-700 focus:ring-indigo-500' }} bg-white dark:bg-slate-900 text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:border-transparent transition-colors">
                            <span class="absolute right-2.5 md:right-3 top-1/2 transform -translate-y-1/2 text-slate-500 dark:text-slate-400 text-sm">мин</span>
                        </div>
                        @error('duration')
                        <p class="mt-2 text-xs text-rose-600 dark:text-rose-400">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        <label for="price" class="flex items-center gap-1.5 md:gap-2 text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">
                            <span>Цена (BYN)*</span>
                        </label>
                        <input type="number" id="price" name="price" required min="0" step="0.01" value="{{ old('price') }}"
                               class="w-full px-2.5 md:px-3 py-2 md:py-2.5 text-sm rounded-md border {{ $errors->has('price') ? 'border-rose-500 focus:ring-rose-500' : 'border-slate-300 dark:border-slate-700 focus:ring-indigo-500' }} bg-white dark:bg-slate-900 text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:border-transparent transition-colors">
                        @error('price')
                        <p class="mt-2 text-xs text-rose-600 dark:text-rose-400">{{ $message }}</p>
                        @enderror
                    </div>
