...
@section('content')
    ...
    <form method="POST" action="{{ route('onboarding.master.store') }}" class="space-y-6">
        ...
        <div class="space-y-5">
            ...
            <div>
                <label for="specialization" class="flex items-center gap-1.5 md:gap-2 text-xs md:text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">
                ...
                <input type="text" id="specialization" name="specialization" required value="{{ old('specialization') }}"
                       class="w-full px-2.5 md:px-3 py-2 md:py-2.5 text-xs md:text-sm rounded-md border {{ $errors->has('specialization') ? 'border-rose-500 focus:ring-rose-500' : 'border-slate-300 dark:border-slate-700 focus:ring-indigo-500' }} bg-white dark:bg-slate-900 text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:border-transparent transition-colors"
