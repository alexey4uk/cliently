        ...
        <div class="space-y-6">
            ...
            <div class="bg-white dark:bg-slate-900 rounded-lg border border-slate-200 dark:border-slate-800 p-6">
                ...
                <div class="space-y-6">
                    ...
                    @endphp
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="space-y-2 md:col-span-2" id="clientPhoneBlockEdit"
                            data-countries="{{ json_encode($countries->map(fn ($c) => ['id' => $c->id, 'code' => $c->calling_code, 'name' => $c->name])->values()) }}"
                            data-old-phone="{{ old('phone', $client->phone) }}"
                            data-old-country="{{ old('phone_country_id', $client->primaryPhone?->country_id) }}">
                                ...
                                <div class="sm:w-48">
                                    <select id="phone_country_id" name="phone_country_id" required
                                        ...
                                        @foreach($countries as $c)
                                            <option value="{{ $c->id }}" data-code="{{ $c->calling_code }}" {{ old('phone_country_id', $client->primaryPhone?->country_id) == $c->id ? 'selected' : '' }}>{{ $c->name }} {{ $c->calling_code }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <div class="flex-1 relative">
                                    <span id="clientPhonePrefixEdit" class="absolute left-4 top-1/2 -translate-y-1/2 text-slate-500 dark:text-slate-400 text-sm pointer-events-none"></span>
                                    ...
                                    <input type="hidden" name="phone" id="client_phone_edit" value="{{ old('phone', $client->phone) }}">
                                </div>
                            </div>
...
@push('scripts')
    <script>
        (function() {
            const block = document.getElementById('clientPhoneBlockEdit');
            const sel = document.getElementById('phone_country_id');
            const national = document.getElementById('phone_national_edit');
            const hidden = document.getElementById('client_phone_edit');
            const prefix = document.getElementById('clientPhonePrefixEdit');
