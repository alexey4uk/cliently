...
@section('content')
    <div class="max-w-[1400px] mx-auto">
        <div x-data="{
        showFilters: {{ $search || $sort !== 'created_at' || $direction !== 'desc' || $perPage != 20 ? 'true' : 'false' }}
    }" class="space-y-6">
            ...
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div class="flex items-center gap-3 sm:gap-4">
                    ...
                    <div class="min-w-0 flex-1">
                        <h1 class="text-xl sm:text-2xl lg:text-3xl font-bold text-slate-900 dark:text-white">Бизнесы</h1>
                        <p class="text-xs sm:text-sm text-slate-600 dark:text-slate-400 mt-0.5 sm:mt-1">Управление бизнесами системы</p>
                    </div>
                </div>
                <div class="flex items-center justify-between sm:justify-end gap-3 sm:gap-4">
                    <div class="text-left sm:text-right">
                        @if(method_exists($businesses, 'total'))
                            <p class="text-2xl sm:text-3xl font-bold text-slate-900 dark:text-white">{{ $businesses->total() }}</p>
                            <p class="text-xs sm:text-sm text-slate-600 dark:text-slate-400">Всего бизнесов</p>
                        @else
                            <p class="text-2xl sm:text-3xl font-bold text-slate-900 dark:text-white">{{ $businesses->count() }}</p>
        ...
        <div class="space-y-4">
            ...
            <form method="GET" action="{{ route('panel.businesses') }}" class="flex flex-col sm:flex-row items-stretch sm:items-end gap-3 sm:gap-4">
                <!-- Поиск -->
                <div class="flex-1 min-w-0">
                    ...
                    <div class="relative">
                        ...
                        <input id="search-input" type="text" name="search" value="{{ $search }}"
                            placeholder="Поиск по названию, телефону или описанию..."
            ...
            <div x-show="showFilters" x-transition:enter="transition ease-out duration-300"
                ...
                style="display: none;">
                <form method="GET" action="{{ route('panel.businesses') }}" class="flex flex-col sm:flex-row sm:flex-wrap items-stretch sm:items-end gap-3 sm:gap-4">
                    ...
                    @if ($search || $sort !== 'created_at' || $direction !== 'desc' || $perPage != 20)
                    ...
                    <table class="w-full">
                        <thead class="bg-slate-50/80 dark:bg-slate-800/30 backdrop-blur-sm border-b border-slate-200 dark:border-slate-700">
                            <tr>
                                ...
                                <th class="px-6 py-4 text-left text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wider">Владелец</th>
                                <th class="px-6 py-4 text-left text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wider">Статистика</th>
                                <th class="px-6 py-4 text-left text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wider">Телефон</th>
                                <th class="px-6 py-4 text-left text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-wider">Дата создания</th>
                                ...
                                <tr class="hover:bg-slate-50/60 dark:hover:bg-slate-800/30 transition-all duration-200">
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="flex items-center gap-3">
                                            ...
                                            <div class="min-w-0">
                                                <p class="text-sm font-semibold text-slate-900 dark:text-white">{{ $business->name }}</p>
                                                @if($business->description)
                                                    <p class="text-xs text-slate-500 dark:text-slate-400 mt-0.5 truncate max-w-xs">{{ $business->description }}</p>
                                                @endif
                                            </div>
                                        </div>
                                    </td>
                                        ...
                                        @if($owner)
                                            <div class="flex items-center gap-2">
                                                ...
                                                <div>
                                                    ...
                                                    <p class="text-xs text-slate-500 dark:text-slate-400">{{ $owner->email ?? '' }}</p>
                                                </div>
                                            </div>
                                        @else
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="flex flex-wrap gap-2">
                                            <span class="inline-flex items-center gap-1 px-2.5 py-1 text-xs font-medium bg-emerald-100 dark:bg-emerald-500/20 text-emerald-700 dark:text-emerald-300 rounded-lg border border-emerald-200 dark:border-emerald-600/30">
                                                <i class="fa-solid fa-users text-xs"></i>
                                                {{ $business->clients_count }} клиентов
                                            </span>
                                            <span class="inline-flex items-center gap-1 px-2.5 py-1 text-xs font-medium bg-indigo-100 dark:bg-indigo-500/20 text-indigo-700 dark:text-indigo-300 rounded-lg border border-indigo-200 dark:border-indigo-600/30">
                                                <i class="fa-solid fa-briefcase text-xs"></i>
                                                {{ $business->services_count }} услуг
                                            </span>
                                            <span class="inline-flex items-center gap-1 px-2.5 py-1 text-xs font-medium bg-amber-100 dark:bg-amber-500/20 text-amber-700 dark:text-amber-300 rounded-lg border border-amber-200 dark:border-amber-600/30">
                                                <i class="fa-solid fa-user-tie text-xs"></i>
                                                {{ $business->masters_count }} мастеров
                                            </span>
                                            @if($business->appointments_count > 0)
                                                <span class="inline-flex items-center gap-1 px-2.5 py-1 text-xs font-medium bg-purple-100 dark:bg-purple-500/20 text-purple-700 dark:text-purple-300 rounded-lg border border-purple-200 dark:border-purple-600/30">
                                                    <i class="fa-solid fa-calendar-check text-xs"></i>
                                                    {{ $business->appointments_count }} записей
                                                </span>
                                            @endif
                                        </div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        @if($business->phone)
                                            <p class="text-sm font-semibold text-slate-900 dark:text-white">{{ $business->phone }}</p>
                                        @else
                                            <span class="text-sm text-slate-400 dark:text-slate-500 italic">Не указан</span>
                                        @endif
                                    </td>
                            ...
                            <div class="flex items-start justify-between gap-3 sm:gap-4">
                                <div class="flex items-center gap-2 sm:gap-3 min-w-0 flex-1">
                                    <div class="h-10 w-10 sm:h-12 sm:w-12 rounded-xl bg-gradient-to-br from-indigo-500 via-indigo-600 to-purple-600 flex items-center justify-center flex-shrink-0 shadow-md">
                                        <i class="fa-solid fa-building text-white text-sm sm:text-base"></i>
                                    </div>
                                    <div class="min-w-0 flex-1">
                        ...
                        <div class="px-4 sm:px-5 py-3 sm:py-4 space-y-3 sm:space-y-4">
                            ...
                            @if($business->description)
                                <div class="flex items-start gap-2 sm:gap-3">
                                    ...
                                    <div class="min-w-0 flex-1">
                                        ...
                                        <p class="text-xs sm:text-sm text-slate-900 dark:text-white">{{ $business->description }}</p>
                                    </div>
                                </div>
                            @endif
                            ...
                            @if($owner)
                                <div class="flex items-center gap-2 sm:gap-3">
                                    <div class="flex-shrink-0">
                                        <div class="h-8 w-8 sm:h-10 sm:w-10 rounded-full bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center text-white font-bold text-xs sm:text-sm shadow-sm">
                                            {{ strtoupper(mb_substr($owner->name ?? ($owner->pivot->first_name ?? ''), 0, 2)) }}
                                        </div>
                                    </div>
                                    <div class="min-w-0 flex-1">
                                        <p class="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wide mb-1.5 sm:mb-2">Владелец</p>
                                        <p class="text-sm font-semibold text-slate-900 dark:text-white">{{ $owner->name ?? ($owner->pivot->first_name . ' ' . ($owner->pivot->last_name ?? '')) }}</p>
                                        @if($owner->email)
                                            <p class="text-xs text-slate-500 dark:text-slate-400 mt-0.5">{{ $owner->email }}</p>
                                        @endif
                                    </div>
                                </div>
                            @endif
                            ...
                            <div class="flex items-start gap-2 sm:gap-3">
                                <div class="flex-shrink-0">
                                    <div class="h-8 w-8 sm:h-10 sm:w-10 rounded-xl bg-purple-100 dark:bg-purple-500/20 flex items-center justify-center shadow-sm">
                                        <i class="fa-solid fa-chart-bar text-purple-600 dark:text-purple-400 text-xs sm:text-sm"></i>
                                    </div>
                                </div>
                                <div class="min-w-0 flex-1">
                                    <p class="text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wide mb-1.5 sm:mb-2">Статистика</p>
                                    <div class="grid grid-cols-2 gap-2">
                                        <div class="flex items-center gap-1.5">
                                            <i class="fa-solid fa-users text-xs text-emerald-600 dark:text-emerald-400"></i>
                                            <span class="text-xs text-slate-600 dark:text-slate-400">{{ $business->clients_count }} клиентов</span>
                                        </div>
                                        <div class="flex items-center gap-1.5">
                                            <i class="fa-solid fa-briefcase text-xs text-indigo-600 dark:text-indigo-400"></i>
                                            <span class="text-xs text-slate-600 dark:text-slate-400">{{ $business->services_count }} услуг</span>
                                        </div>
                                        <div class="flex items-center gap-1.5">
                                            <i class="fa-solid fa-user-tie text-xs text-amber-600 dark:text-amber-400"></i>
                                            <span class="text-xs text-slate-600 dark:text-slate-400">{{ $business->masters_count }} мастеров</span>
                                        </div>
                                        <div class="flex items-center gap-1.5">
                                            <i class="fa-solid fa-map-marker-alt text-xs text-blue-600 dark:text-blue-400"></i>
                                            <span class="text-xs text-slate-600 dark:text-slate-400">{{ $business->locations_count }} локаций</span>
                                        </div>
                                        @if($business->appointments_count > 0)
                                            <div class="flex items-center gap-1.5 col-span-2">
                                                <i class="fa-solid fa-calendar-check text-xs text-purple-600 dark:text-purple-400"></i>
                                                <span class="text-xs text-slate-600 dark:text-slate-400">{{ $business->appointments_count }} записей</span>
                                            </div>
                                        @endif
                                    </div>
                                </div>
                            ...
                            <div class="flex items-center gap-2 sm:gap-3">
                                ...
                                <div class="min-w-0 flex-1">
                                    ...
                                    <p class="text-sm font-semibold text-slate-900 dark:text-white">{{ $business->created_at->format('d.m.Y H:i') }}</p>
                                </div>
                            </div>
                        </div>
                    </x-mobile-card>
                ...
                <div class="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm px-4 sm:px-6 py-4 sm:py-5">
                    <div class="flex flex-col lg:flex-row items-center justify-between gap-4 sm:gap-5">
                        <div class="text-xs sm:text-sm text-slate-600 dark:text-slate-400 text-center sm:text-left">
                            ...
                            @if(method_exists($businesses, 'total'))
                                <span class="font-bold text-slate-900 dark:text-white">{{ $businesses->firstItem() }}</span>
                                <span class="font-medium">—</span>
                                <span class="font-bold text-slate-900 dark:text-white">{{ $businesses->lastItem() }}</span>
                                <span class="font-medium">из</span>
                                <span class="font-bold text-slate-900 dark:text-white">{{ $businesses->total() }}</span>
                                <span class="font-medium">бизнесов</span>
                            @else
                                <span class="font-medium">Страница {{ $businesses->currentPage() }}</span>
                            @endif
                        </div>
                        ...
                        <div class="flex items-center gap-1 flex-wrap justify-center">
                            ...
                            @if ($businesses->hasMorePages())
                                <a href="{{ $businesses->nextPageUrl() }}" class="w-9 h-9 sm:w-11 sm:h-11 flex items-center justify-center bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-700 hover:border-slate-400 dark:hover:border-slate-600 transition-all duration-200 text-slate-700 dark:text-slate-300 shadow-sm hover:shadow-md">
            ...
            <div class="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm p-8 sm:p-12 md:p-16 text-center">
                <div class="max-w-md mx-auto">
                    <div class="h-16 w-16 sm:h-20 sm:w-20 rounded-2xl bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center mx-auto mb-4 sm:mb-6 shadow-lg">
                        <i class="fa-solid fa-building text-white text-2xl sm:text-3xl"></i>
                    </div>
                    ...
                    <p class="text-sm sm:text-base text-slate-600 dark:text-slate-400 mb-6 sm:mb-8 leading-relaxed px-2">
                        ...
                        @else
                            Бизнесы будут отображаться здесь после их создания в системе
                        @endif
                    </p>
                    @if ($search)
                        <a href="{{ route('panel.businesses') }}" class="inline-flex items-center justify-center gap-2 px-5 sm:px-6 py-2.5 sm:py-3 text-xs sm:text-sm font-semibold text-slate-700 dark:text-slate-300 bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-xl hover:bg-slate-100 dark:hover:bg-slate-800 hover:border-slate-300 dark:hover:border-slate-600 transition-all duration-200 shadow-sm">
                            <i class="fa-solid fa-rotate-left text-xs sm:text-sm"></i>
                            <span>Сбросить фильтры</span>
                        </a>
                    @endif
