    ...
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 pb-20 sm:pb-8">
        ...
        <nav class="mb-4 sm:mb-6" aria-label="Breadcrumb">
            <ol class="flex items-center space-x-1 sm:space-x-2 text-xs sm:text-sm text-slate-500 dark:text-slate-400 overflow-x-auto">
                ...
                <li class="flex-shrink-0"><i class="fa-solid fa-chevron-right text-xs"></i></li>
                <li class="flex-shrink-0 text-slate-900 dark:text-white font-medium">Создание</li>
            </ol>
        </nav>
        ...
        <div class="mb-6 sm:mb-8">
            <div class="flex items-start sm:items-center gap-3 sm:gap-4">
                ...
                <div class="min-w-0 flex-1">
                    <h1 class="text-xl sm:text-2xl lg:text-3xl font-bold text-slate-900 dark:text-white mb-1">Создание свойства</h1>
                    <p class="text-xs sm:text-sm text-slate-600 dark:text-slate-400">Создайте новое свойство для использования в тарифах</p>
                </div>
            </div>
        </div>

        <!-- Form -->
        <form method="POST" action="{{ route('panel.plans.properties.store') }}" id="property-form" class="space-y-6">
            @csrf
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                        <div>
                            ...
                            @if(!$errors->has('key'))
                                <p id="key-help" class="mt-2 text-xs text-slate-500 dark:text-slate-400">
                                    ...
                                    Только строчные буквы, цифры и подчеркивания (snake_case). Например: max_locations, analytics_enabled
                                </p>
                            @endif
                            @error('key')
                        ...
                        <div>
                            ...
                            <select
                                ...
                                <option value="integer" {{ old('type') === 'integer' ? 'selected' : '' }}>Число (integer)</option>
                                <option value="boolean" {{ old('type') === 'boolean' ? 'selected' : '' }}>Да/Нет (boolean)</option>
                            </select>
                            @error('type')
                    ...
                    <div>
                        ...
                        <div class="mt-2 flex items-center justify-between">
                            ...
                            <span id="description-count" class="text-xs text-slate-400 dark:text-slate-500">
                                <span id="description-length">{{ mb_strlen(old('description', '')) }}</span>/500
                            </span>
                        </div>
                        @error('description')
                            <p class="mt-2 text-xs text-rose-600 dark:text-rose-400 flex items-center gap-1" role="alert">
            ...
            <div class="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden">
                ...
                <div class="p-4 sm:p-6 space-y-5">
                        ...
                        <div class="relative">
                            ...
                            @if(old('icon'))
                                <div class="absolute right-3 top-1/2 -translate-y-1/2">
                                    <i class="{{ old('icon') }} text-slate-400 text-lg"></i>
                                </div>
                            @endif
                        </div>
                    ...
                    <div>
                        <label class="flex items-start gap-3 p-4 rounded-lg border-2 border-slate-200 dark:border-slate-700 hover:border-indigo-300 dark:hover:border-indigo-600 bg-slate-50 dark:bg-slate-800/50 cursor-pointer transition-all group">
                            <input
                                type="checkbox"
                                id="is_active"
                                name="is_active"
                    ...
                    <div class="py-4 sm:py-6">
                        <div class="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 sm:gap-4">
                            <a
                                ...
                                class="order-2 sm:order-1 inline-flex items-center justify-center gap-2 px-4 sm:px-5 py-3 bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 rounded-xl font-medium transition-all shadow-sm hover:shadow-md text-sm sm:text-base">
                        ...
                        <p class="mt-3 sm:hidden text-center text-xs text-slate-500 dark:text-slate-400">
                            <i class="fa-solid fa-info-circle mr-1"></i>
                            Проверьте все поля перед созданием
                        </p>
                    </div>
                </div>
            </div>
            ...
            if (form) {
                form.addEventListener('submit', function() {
                    ...
                    if (submitBtn) {
                        ...
                        submitBtn.innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i> <span>Создание...</span>';
                    }
                });
            }
        });
