...
@section('title', 'Просмотр локации')
            ...
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div class="flex items-center gap-3 sm:gap-4">
                    ...
                    <div class="min-w-0 flex-1">
                        <h1 class="text-xl sm:text-2xl lg:text-3xl font-bold text-slate-900 dark:text-white">{{ $location->name }}</h1>
                        <p class="text-xs sm:text-sm text-slate-600 dark:text-slate-400 mt-0.5 sm:mt-1">Детальная информация о локации</p>
                    </div>
                </div>
        ...
        <div class="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm p-4 sm:p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
                ...
                <div>
                    ...
                    <p class="text-sm sm:text-base font-semibold text-slate-900 dark:text-white">{{ $location->name }}</p>
                </div>
                ...
                <div>
                    ...
                    @if($location->business)
                        <p class="text-sm sm:text-base font-semibold text-slate-900 dark:text-white">{{ $location->business->name }}</p>
                    @else
                ...
                <div class="md:col-span-2">
                    <p class="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide mb-2">Адрес</p>
                    <p class="text-sm sm:text-base text-slate-900 dark:text-white">{{ $location->full_address }}</p>
                    <div class="mt-2 text-xs text-slate-500 dark:text-slate-400">
                        @if($location->city)
                            <span>Город: {{ $location->city }}</span>
                        @endif
                        @if($location->street)
                            <span class="mx-2">•</span>
                            <span>Улица: {{ $location->street }}</span>
                        @endif
                        @if($location->house)
                            <span class="mx-2">•</span>
                            <span>Дом: {{ $location->house }}</span>
                        @endif
                        @if($location->building)
                            <span class="mx-2">•</span>
                            <span>Корпус: {{ $location->building }}</span>
                        @endif
                        @if($location->apartment)
                            <span class="mx-2">•</span>
                            <span>Квартира/Офис: {{ $location->apartment }}</span>
                        @endif
                    </div>
                </div>
                ...
                <div>
                    ...
                    @if($location->phone)
                        <p class="text-sm sm:text-base font-semibold text-slate-900 dark:text-white">{{ $location->phone }}</p>
                    @else
                        <p class="text-sm sm:text-base text-slate-400 dark:text-slate-500 italic">Не указан</p>
                    @endif
                </div>
                ...
                @if($location->description)
                    <div class="md:col-span-2">
                        ...
                        <p class="text-sm sm:text-base text-slate-900 dark:text-white">{{ $location->description }}</p>
                    </div>
                @endif
                ...
                <div>
                    ...
                    <p class="text-sm sm:text-base text-slate-900 dark:text-white">{{ $location->created_at->format('d.m.Y H:i') }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm p-4 sm:p-6">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                ...
                <div class="bg-slate-50 dark:bg-slate-800/50 rounded-lg p-4 border border-slate-200 dark:border-slate-700">
                    ...
                    <p class="text-2xl font-bold text-slate-900 dark:text-white">{{ $location->masters_count }}</p>
                </div>
                ...
                <div class="bg-slate-50 dark:bg-slate-800/50 rounded-lg p-4 border border-slate-200 dark:border-slate-700">
                    ...
                    <p class="text-2xl font-bold text-slate-900 dark:text-white">{{ $appointmentsCount }}</p>
                </div>
            </div>
        </div>
    </div>
