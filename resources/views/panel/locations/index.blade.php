    ...
    <div class="max-w-350 mx-auto">
        ...
        <div class="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm p-4 sm:p-6">
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div class="flex items-center gap-3 sm:gap-4">
                    <div class="h-10 w-10 sm:h-12 sm:w-12 rounded-xl bg-linear-to-br from-indigo-500 to-purple-600 flex items-center justify-center shadow-sm shrink-0">
        ...
        <div class="space-y-4">
            ...
            <form method="GET" action="{{ route('panel.locations') }}" class="flex flex-col sm:flex-row items-stretch sm:items-end gap-3 sm:gap-4">
                ...
                <!-- Кнопка поиска -->
                <button type="submit" class="px-4 sm:px-6 py-2.5 sm:py-3 bg-linear-to-r from-indigo-600 to-indigo-700 text-white text-xs sm:text-sm font-medium rounded-xl hover:from-indigo-700 hover:to-indigo-800 transition-all duration-200 shrink-0 shadow-sm hover:shadow-md">
        ...
        @if ($locations->count() > 0)
            ...
            <div class="hidden md:block">
                <div class="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden">
                    <table class="w-full">
                        <thead class="bg-linear-to-r from-slate-50 to-slate-100/50 dark:from-slate-800/50 dark:to-slate-800/30 backdrop-blur-sm border-b-2 border-slate-200 dark:border-slate-700">
                            <tr>
                        ...
                        <tbody class="divide-y divide-slate-200 dark:divide-slate-800">
                            @foreach ($locations as $location)
                                <tr class="hover:bg-slate-50/80 dark:hover:bg-slate-800/50 transition-all duration-200 group">
                                    <td class="px-4 py-3">
                                        <div class="min-w-0">
                                            <p class="text-sm font-semibold text-slate-900 dark:text-white group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors truncate max-w-50">{{ $location->name }}</p>
                                            @if($location->description)
                                                <p class="text-xs text-slate-500 dark:text-slate-400 mt-0.5 truncate max-w-50">{{ $location->description }}</p>
                                            @endif
                                        </div>
                                    </td>
                                    <td class="px-4 py-3">
                                        <div class="min-w-0 max-w-62.5">
                                            <p class="text-sm text-slate-900 dark:text-white truncate">{{ $location->full_address }}</p>
                                        </div>
                                    </td>
                                    <td class="px-4 py-3">
                                        @if($location->business)
                                            <p class="text-sm text-slate-900 dark:text-white truncate max-w-45">{{ $location->business->name }}</p>
                                        @else
            ...
            <div class="md:hidden grid grid-cols-1 gap-4">
                @foreach ($locations as $location)
                    <x-mobile-card>
                        <!-- Заголовок карточки -->
                        <div class="px-4 sm:px-5 py-3 sm:py-4 border-b border-slate-200 dark:border-slate-700 bg-linear-to-r from-slate-50 to-slate-100/50 dark:from-slate-800/30 dark:to-slate-800/20">
                            <div class="flex items-start justify-between gap-3 sm:gap-4">
                                <div class="flex items-center gap-2 sm:gap-3 min-w-0 flex-1">
                                    <div class="h-10 w-10 sm:h-12 sm:w-12 rounded-xl bg-linear-to-br from-indigo-500 via-indigo-600 to-purple-600 flex items-center justify-center shrink-0 shadow-md">
        ...
        @else
            ...
            <div class="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm p-8 sm:p-12 md:p-16 text-center">
                <div class="max-w-md mx-auto">
                    <div class="h-16 w-16 sm:h-20 sm:w-20 rounded-2xl bg-linear-to-br from-indigo-500 to-purple-600 flex items-center justify-center mx-auto mb-4 sm:mb-6 shadow-lg">
