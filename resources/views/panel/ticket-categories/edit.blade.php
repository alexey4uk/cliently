    ...
    <div class="max-w-2xl mx-auto">
        ...
        <div class="flex items-center justify-between">
            <div>
                ...
                <p class="text-slate-600 dark:text-slate-400 mt-1">Измените информацию о категории</p>
            </div>
        ...
        <form method="POST" action="{{ route('panel.ticket-categories.update', $ticketCategory) }}">
            @csrf
            @method('PATCH')
            <div class="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 p-6 md:p-8 shadow-sm space-y-6">
                ...
                <div>
                    ...
                    <textarea name="description" rows="4" 
                        ...
                        class="w-full px-4 py-3 rounded-lg border-slate-300 dark:border-slate-700 dark:bg-slate-800 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-colors resize-none">{{ old('description', $ticketCategory->description) }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="number" name="sort_order" value="{{ old('sort_order', $ticketCategory->sort_order) }}" min="0"
                            class="w-full px-4 py-3 rounded-lg border-slate-300 dark:border-slate-700 dark:bg-slate-800 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 transition-colors">
                    ...
                    <div class="flex items-center">
                        <label class="flex items-center gap-3 cursor-pointer">
                            <input type="checkbox" name="is_active" value="1" {{ old('is_active', $ticketCategory->is_active) ? 'checked' : '' }}
