...
@section('title', 'Создать роль бизнеса')

@push('breadcrumbs')
    <x-breadcrumbs :base="['title' => 'Главная', 'url' => route('panel.index')]" :items="[['title' => 'Роли и доступы', 'url' => null], ['title' => 'Роли бизнеса', 'url' => route('panel.business-roles.index')], ['title' => 'Создать роль', 'url' => null]]" />
@endpush
    ...
    <div class="max-w-5xl mx-auto space-y-6">
        <div x-data="{
            ...
            selectAllInSubgroup(circuit, subgroup) {
                ...
                if (allSelected) {
                    perms.forEach(p => {
                        const i = this.selectedPermissions.indexOf(p);
            ...
            getPermissionGroups() {
                ...
                const subgroupLabels = {{ json_encode([
                    ...
                    'telegram' => 'Telegram', 'tickets' => 'Тикеты', 'online_booking' => 'Онлайн-запись',
                ]) }};
                ...
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    <div class="flex items-center gap-3 sm:gap-4">
                        ...
                        <div class="min-w-0 flex-1">
                            <h1 class="text-xl sm:text-2xl lg:text-3xl font-bold text-slate-900 dark:text-white">Создание роли бизнеса</h1>
                            <p class="text-sm text-slate-600 dark:text-slate-400 mt-1">Код, название, описание и права. Только клиентский контур (client.*). Роль будет доступна всем бизнесам.</p>
                        </div>
                    </div>
                    <a href="{{ route('panel.business-roles.index') }}"
                       class="inline-flex items-center justify-center gap-2 px-4 sm:px-6 py-2.5 sm:py-3 bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-300 rounded-xl text-xs sm:text-sm font-medium transition-colors shadow-sm flex-shrink-0">
            ...
            <form method="POST" action="{{ route('panel.business-roles.store') }}" class="space-y-6">
                ...
                <div class="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm p-4 sm:p-6">
                    ...
                    <div class="space-y-4 sm:space-y-6">
                        <div>
                            <label for="slug" class="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2 uppercase tracking-wide">Код роли *</label>
                            <div class="relative">
                                <div class="absolute inset-y-0 left-0 pl-3 sm:pl-4 flex items-center pointer-events-none">
                                    <i class="fa-solid fa-code text-slate-400 text-sm"></i>
                                </div>
                                <input type="text" id="slug" name="slug" value="{{ old('slug') }}" placeholder="например: manager"
                                       class="w-full pl-10 sm:pl-11 pr-4 py-2.5 sm:py-3 rounded-xl border {{ $errors->has('slug') ? 'border-rose-500 focus:ring-rose-500' : 'border-slate-300 dark:border-slate-700 focus:ring-indigo-500' }} bg-white dark:bg-slate-900 text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:border-transparent transition-colors text-sm shadow-sm">
                            </div>
                            @error('slug')<p class="mt-1.5 text-xs text-rose-600 dark:text-rose-400">{{ $message }}</p>@enderror
                            <p class="mt-1.5 text-xs text-slate-500 dark:text-slate-400">Латиница, цифры и подчёркивания. Начинайте с буквы.</p>
                        </div>
                        <div>
                            <label for="name" class="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2 uppercase tracking-wide">Название роли *</label>
                            <div class="relative">
                                <div class="absolute inset-y-0 left-0 pl-3 sm:pl-4 flex items-center pointer-events-none">
                                    <i class="fa-solid fa-tag text-slate-400 text-sm"></i>
                                </div>
                                <input type="text" id="name" name="name" value="{{ old('name') }}" placeholder="например: Менеджер"
                        ...
                        <div>
                            <label for="description" class="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-2 uppercase tracking-wide">Описание</label>
                            <div class="relative">
                                <div class="absolute inset-y-0 left-0 pl-3 sm:pl-4 flex items-center pointer-events-none">
                                    <i class="fa-solid fa-align-left text-slate-400 text-sm"></i>
                                </div>
                                <input type="text" id="description" name="description" value="{{ old('description') }}" placeholder="например: Управляет клиентами и записями"
                ...
                <div class="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm p-4 sm:p-6">
                    ...
                    <!-- Контур: Клиентский контур → подгруппы -->
