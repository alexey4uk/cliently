            ...
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div class="flex items-center gap-3 sm:gap-4">
                    ...
                    <div class="min-w-0 flex-1">
                        <h1 class="text-xl sm:text-2xl lg:text-3xl font-bold text-slate-900 dark:text-white">{{ $service->name }}</h1>
                        <p class="text-xs sm:text-sm text-slate-600 dark:text-slate-400 mt-0.5 sm:mt-1">Детальная информация об услуге</p>
                    </div>
                </div>
        ...
        <div class="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm p-4 sm:p-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
                ...
                <div>
                    ...
                    <p class="text-sm sm:text-base font-semibold text-slate-900 dark:text-white">{{ $service->name }}</p>
                </div>
                ...
                <div>
                    ...
                    @if($service->business)
                        <p class="text-sm sm:text-base font-semibold text-slate-900 dark:text-white">{{ $service->business->name }}</p>
                    @else
                ...
                @if($service->description)
                    <div class="md:col-span-2">
                        ...
                        <p class="text-sm sm:text-base text-slate-900 dark:text-white">{{ $service->description }}</p>
                    </div>
                @endif
                ...
                <!-- Длительность -->
                <div>
                    <p class="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide mb-2">Длительность</p>
                    <p class="text-sm sm:text-base font-semibold text-slate-900 dark:text-white">{{ $service->duration }} минут</p>
                </div>
                ...
                <div>
                    ...
                    <p class="text-sm sm:text-base text-slate-900 dark:text-white">{{ $service->created_at->format('d.m.Y H:i') }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm p-4 sm:p-6">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                <div class="bg-slate-50 dark:bg-slate-800/50 rounded-lg p-4 border border-slate-200 dark:border-slate-700">
                    ...
                    <p class="text-2xl font-bold text-slate-900 dark:text-white">{{ $service->appointments->count() }}</p>
                </div>
            </div>
        </div>
    </div>
